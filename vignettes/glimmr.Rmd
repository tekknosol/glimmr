...
created: One continous dataset containing the raw gas mixing ratios and one 
metadataset with information about the single applications. Typically this 
...
the metadata and pases this to `gasfluxes::gasfluxes` for the actual flux 
calculation.
...
## Process fluxes
To use any arbitrary device all columns containing used informations need to be
defined.
```{r, eval=FALSE}
process_chamber(
  # specify data
...
While the start of the single measurements is (usually) defined by a known point
